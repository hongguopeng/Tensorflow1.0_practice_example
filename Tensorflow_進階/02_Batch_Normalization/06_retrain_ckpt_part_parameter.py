...
train_variable_name = []
for name in range(0 , len(train_variable_list)):
    train_variable_name.append(train_variable_list[name].name)
    
# 可以針對不同的weight與bias及其他trainable variable去做不同的訓練，
# 在這裡只讓前16個trainable variable(layer1~layer4)去做訓練，
# 而另外的trainable variable(layer5~layer7、prediction)的learn rate調整為0，
# 所以這些trainable variable不會改變，進而達到fine tuning的效果
train_op_part1 = tf.train.GradientDescentOptimizer(0.001).minimize(cost , var_list = train_variable_list[0 : 16])
train_op_part2 = tf.train.GradientDescentOptimizer(0).minimize(cost , var_list = train_variable_list[16 : 30])
train_op = tf.group(train_op_part1 , train_op_part2)
#----------------------original network----------------------# 
...
# 用這樣的方式讀取，需要將原來的network再建立一次，比較麻煩
#----------------------model restore----------------------# 
...
saver.save(sess , 'my_network_retrain_part_parameter/save_net_retrain_part_parameter')
